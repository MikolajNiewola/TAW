...
const dataEndpoint = (router) => {
    router.get('/api/posts', async (request, response, next) => {
        ...
        if(request.query.title) {
            posts.forEach(post =>{
                if(post.title.includes(request.query.title)) {
                    postsToDisplay.push(post);
                }
            });
        } else {
            for(let i=0; i<limit; i++) {
                if(posts[i+(page-1)*limit]){
                    postsToDisplay.push(posts[i+(page-1)*limit]);
                }
            }
        }
        
        response.status(200).send({posts: postsToDisplay});
    });
    ...
    router.get('/api/post/:id', async (request, response, next) => {
        response.status(200).send({posts: posts[posts.findIndex(post => post.id == request.params.id)]});
    })

    router.put('/api/post/:id', async (request, response, next) => {
        posts[posts.findIndex(post => post.id == request.params.id)] = request.body.newPost;

        response.status(200).send({posts: posts[posts.findIndex(post => post.id == request.params.id)]});
    })

    router.delete('/api/post/:id', async (request, response, next) => {
        const idToRemove = posts.findIndex(post => post.id == request.params.id);
